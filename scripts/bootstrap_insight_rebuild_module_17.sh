...
# Insight Rebuild Module 17 official bootstrap script.
# Policy: do not use set -e; terminal output stays short; detailed logs are written as GitHub evidence.

VERSION="2026-06-25-insight-rebuild-module-17-official"
...
summary "Insight Rebuild Module 17 official bootstrap $VERSION starting..."
...
git fetch origin >/tmp/insight_module_17_git.log 2>&1
git checkout "$BRANCH" >>/tmp/insight_module_17_git.log 2>&1
git pull --ff-only origin "$BRANCH" >>/tmp/insight_module_17_git.log 2>&1
rc=$?
...
LOG_FILE="$PRODUCT_DIR/reports/execution/insight_rebuild_module_17_bootstrap_latest.log"
{
  echo "# Insight Rebuild Module 17 bootstrap log"
  ...
  echo ""
  cat /tmp/insight_module_17_git.log 2>/dev/null || true
} > "$LOG_FILE" 2>&1
rm -f /tmp/insight_module_17_git.log

DUPLICATE_BOOTSTRAPS="$(find scripts -maxdepth 1 -type f -name 'bootstrap_insight_rebuild_module_17*.sh' ! -name 'bootstrap_insight_rebuild_module_17.sh' -print 2>> "$LOG_FILE")"
if [ -n "$DUPLICATE_BOOTSTRAPS" ]; then
  echo "duplicate_insight_rebuild_module_17_bootstraps=$DUPLICATE_BOOTSTRAPS" >> "$LOG_FILE"
  fail_short "duplicate Insight Rebuild Module 17 bootstrap entrypoints found; keep one official entry only"
fi

summary "Running Module 17 unit tests quietly..."
...
result = {
    "module": "Insight Rebuild Module 17",
    ...
    "reuse_adoption_boundary": True,
    "custom_code_default": False,
    "harness_upgrade_recommended": True,
    "agents_md_reuse_recommended": True,
    "candidate_types": ["github_skill", "openclaw_skill", "harness", "agents_md"],
    "reuse_adoption_version": "2026-06-25-module-17",
    "expected_unit_test_count": 63,
    "log_file": os.environ.get("LOG_FILE", ""),
    "checks": [
        "prior_module_test_compatibility",
        "high_quality_candidate_adopted",
        "unapproved_license_rejected",
        "network_secret_candidate_not_direct_adopt",
        "default_candidates_include_harness_and_agents_md",
        "adoption_plan_custom_code_default_false",
        "summary_reports_reuse_boundary_counts",
        "score_penalizes_network_and_secret"
    ]
}
Path("reports/testing/insight_rebuild_module_17_test_result.json").write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
Path("reports/testing/latest_test_result.json").write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
PY

cat > reports/execution/insight_rebuild_module_17_execution_report.md <<EOF
...
ensure_git_identity
git add reports/testing/insight_rebuild_module_17_test_result.json reports/testing/latest_test_result.json reports/execution/insight_rebuild_module_17_execution_report.md reports/execution/insight_rebuild_module_17_bootstrap_latest.log >> "$LOG_FILE" 2>&1
git commit -m "insight-rebuild(module-17): add reuse adoption evidence" >> "$LOG_FILE" 2>&1
rc=$?
if [ "$rc" -ne 0 ]; then fail_short "git commit failed for Module 17 evidence"; fi
EVIDENCE_SHA="$(git rev-parse HEAD 2>> "$LOG_FILE")"
"$PYTHON_BIN" - <<PY >> "$LOG_FILE" 2>&1
from pathlib import Path
p = Path("reports/execution/insight_rebuild_module_17_execution_report.md")
text = p.read_text(encoding="utf-8").replace("Pending until evidence commit completes.", "$EVIDENCE_SHA")
p.write_text(text, encoding="utf-8")
PY
git add reports/execution/insight_rebuild_module_17_execution_report.md reports/execution/insight_rebuild_module_17_bootstrap_latest.log >> "$LOG_FILE" 2>&1
git commit -m "insight-rebuild(module-17): record reuse adoption evidence commit sha" >> "$LOG_FILE" 2>&1
...
if [ "$TEST_RC" -eq 0 ]; then
  summary "DONE: Insight Rebuild Module 17 tests passed. Evidence pushed."
  summary "Commit: $FINAL_SHA"
  summary "Evidence: reports/testing/latest_test_result.json; reports/execution/insight_rebuild_module_17_execution_report.md"
  exit 0
fi
summary "FAILED: Insight Rebuild Module 17 tests failed. Evidence pushed."
summary "Commit: $FINAL_SHA"
summary "Evidence: reports/testing/latest_test_result.json; reports/execution/insight_rebuild_module_17_execution_report.md"
exit "$TEST_RC"
