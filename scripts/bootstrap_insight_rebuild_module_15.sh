...
# Insight Rebuild Module 15 official bootstrap script.
# Policy: do not use set -e; terminal output stays short; detailed logs are written as GitHub evidence.

VERSION="2026-06-24-insight-rebuild-module-15-official"
...
summary "Insight Rebuild Module 15 official bootstrap $VERSION starting..."
...
git fetch origin >/tmp/insight_module_15_git.log 2>&1
git checkout "$BRANCH" >>/tmp/insight_module_15_git.log 2>&1
git pull --ff-only origin "$BRANCH" >>/tmp/insight_module_15_git.log 2>&1
rc=$?
...
LOG_FILE="$PRODUCT_DIR/reports/execution/insight_rebuild_module_15_bootstrap_latest.log"
{
  echo "# Insight Rebuild Module 15 bootstrap log"
  ...
  echo ""
  cat /tmp/insight_module_15_git.log 2>/dev/null || true
} > "$LOG_FILE" 2>&1
rm -f /tmp/insight_module_15_git.log

DUPLICATE_BOOTSTRAPS="$(find scripts -maxdepth 1 -type f -name 'bootstrap_insight_rebuild_module_15*.sh' ! -name 'bootstrap_insight_rebuild_module_15.sh' -print 2>> "$LOG_FILE")"
if [ -n "$DUPLICATE_BOOTSTRAPS" ]; then
  echo "duplicate_insight_rebuild_module_15_bootstraps=$DUPLICATE_BOOTSTRAPS" >> "$LOG_FILE"
  fail_short "duplicate Insight Rebuild Module 15 bootstrap entrypoints found; keep one official entry only"
fi

summary "Running Module 15 unit tests quietly..."
...
result = {
    "module": "Insight Rebuild Module 15",
    ...
    "remote_runtime_queue_boundary": True,
    "default_local_runtime_safe": True,
    "remote_boundary_adapter": True,
    "credential_redaction": True,
    "remote_dispatch_attempted": False,
    "dispatch_implemented": False,
    "queue_version": "2026-06-24-module-15",
    "expected_unit_test_count": 49,
    "log_file": os.environ.get("LOG_FILE", ""),
    "checks": [
        "prior_module_test_compatibility",
        "default_queue_adapter_disabled_local_safe",
        "disabled_adapter_enqueue_does_not_dispatch",
        "remote_boundary_missing_endpoint",
        "remote_boundary_missing_credential",
        "remote_boundary_configured_not_dispatched",
        "dispatch_enabled_blocked_until_implemented",
        "summary_redacts_credentials",
        "job_id_is_deterministic"
    ]
}
Path("reports/testing/insight_rebuild_module_15_test_result.json").write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
Path("reports/testing/latest_test_result.json").write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
PY

cat > reports/execution/insight_rebuild_module_15_execution_report.md <<EOF
...
ensure_git_identity
git add reports/testing/insight_rebuild_module_15_test_result.json reports/testing/latest_test_result.json reports/execution/insight_rebuild_module_15_execution_report.md reports/execution/insight_rebuild_module_15_bootstrap_latest.log >> "$LOG_FILE" 2>&1
git commit -m "insight-rebuild(module-15): add remote runtime queue evidence" >> "$LOG_FILE" 2>&1
rc=$?
if [ "$rc" -ne 0 ]; then fail_short "git commit failed for Module 15 evidence"; fi
EVIDENCE_SHA="$(git rev-parse HEAD 2>> "$LOG_FILE")"
"$PYTHON_BIN" - <<PY >> "$LOG_FILE" 2>&1
from pathlib import Path
p = Path("reports/execution/insight_rebuild_module_15_execution_report.md")
text = p.read_text(encoding="utf-8").replace("Pending until evidence commit completes.", "$EVIDENCE_SHA")
p.write_text(text, encoding="utf-8")
PY
git add reports/execution/insight_rebuild_module_15_execution_report.md reports/execution/insight_rebuild_module_15_bootstrap_latest.log >> "$LOG_FILE" 2>&1
git commit -m "insight-rebuild(module-15): record remote runtime queue evidence commit sha" >> "$LOG_FILE" 2>&1
...
if [ "$TEST_RC" -eq 0 ]; then
  summary "DONE: Insight Rebuild Module 15 tests passed. Evidence pushed."
  summary "Commit: $FINAL_SHA"
  summary "Evidence: reports/testing/latest_test_result.json; reports/execution/insight_rebuild_module_15_execution_report.md"
  exit 0
fi
summary "FAILED: Insight Rebuild Module 15 tests failed. Evidence pushed."
summary "Commit: $FINAL_SHA"
summary "Evidence: reports/testing/latest_test_result.json; reports/execution/insight_rebuild_module_15_execution_report.md"
exit "$TEST_RC"
