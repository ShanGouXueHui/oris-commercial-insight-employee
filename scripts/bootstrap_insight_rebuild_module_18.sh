...
# Insight Rebuild Module 18 official bootstrap script.
# Policy: do not use set -e; terminal output stays short; detailed logs are written as GitHub evidence.

VERSION="2026-06-25-insight-rebuild-module-18-official"
...
summary "Insight Rebuild Module 18 official bootstrap $VERSION starting..."
...
git fetch origin >/tmp/insight_module_18_git.log 2>&1
git checkout "$BRANCH" >>/tmp/insight_module_18_git.log 2>&1
git pull --ff-only origin "$BRANCH" >>/tmp/insight_module_18_git.log 2>&1
rc=$?
...
LOG_FILE="$PRODUCT_DIR/reports/execution/insight_rebuild_module_18_bootstrap_latest.log"
{
  echo "# Insight Rebuild Module 18 bootstrap log"
  ...
  echo ""
  cat /tmp/insight_module_18_git.log 2>/dev/null || true
} > "$LOG_FILE" 2>&1
rm -f /tmp/insight_module_18_git.log

DUPLICATE_BOOTSTRAPS="$(find scripts -maxdepth 1 -type f -name 'bootstrap_insight_rebuild_module_18*.sh' ! -name 'bootstrap_insight_rebuild_module_18.sh' -print 2>> "$LOG_FILE")"
if [ -n "$DUPLICATE_BOOTSTRAPS" ]; then
  echo "duplicate_insight_rebuild_module_18_bootstraps=$DUPLICATE_BOOTSTRAPS" >> "$LOG_FILE"
  fail_short "duplicate Insight Rebuild Module 18 bootstrap entrypoints found; keep one official entry only"
fi

summary "Running Module 18 unit tests quietly..."
...
result = {
    "module": "Insight Rebuild Module 18",
    ...
    "loop_engineering_boundary": True,
    "oris_page_loop": True,
    "harness_upgrade_loop": True,
    "budget_gate_required": True,
    "evidence_gate_required": True,
    "human_acceptance_required": True,
    "infinite_loop_allowed": False,
    "loop_engineering_version": "2026-06-25-module-18",
    "expected_unit_test_count": 71,
    "log_file": os.environ.get("LOG_FILE", ""),
    "checks": [
        "prior_module_test_compatibility",
        "default_loops_include_oris_page_and_harness_upgrade",
        "bounded_loop_enabled",
        "unbounded_loop_rejected",
        "missing_evidence_gate_rejected",
        "network_secret_loop_deferred",
        "loop_plan_requires_budget_and_evidence_gates",
        "summary_reports_loop_boundary",
        "bounded_helper_enforces_limits"
    ]
}
Path("reports/testing/insight_rebuild_module_18_test_result.json").write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
Path("reports/testing/latest_test_result.json").write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
PY

cat > reports/execution/insight_rebuild_module_18_execution_report.md <<EOF
...
ensure_git_identity
git add reports/testing/insight_rebuild_module_18_test_result.json reports/testing/latest_test_result.json reports/execution/insight_rebuild_module_18_execution_report.md reports/execution/insight_rebuild_module_18_bootstrap_latest.log >> "$LOG_FILE" 2>&1
git commit -m "insight-rebuild(module-18): add loop engineering evidence" >> "$LOG_FILE" 2>&1
rc=$?
if [ "$rc" -ne 0 ]; then fail_short "git commit failed for Module 18 evidence"; fi
EVIDENCE_SHA="$(git rev-parse HEAD 2>> "$LOG_FILE")"
"$PYTHON_BIN" - <<PY >> "$LOG_FILE" 2>&1
from pathlib import Path
p = Path("reports/execution/insight_rebuild_module_18_execution_report.md")
text = p.read_text(encoding="utf-8").replace("Pending until evidence commit completes.", "$EVIDENCE_SHA")
p.write_text(text, encoding="utf-8")
PY
git add reports/execution/insight_rebuild_module_18_execution_report.md reports/execution/insight_rebuild_module_18_bootstrap_latest.log >> "$LOG_FILE" 2>&1
git commit -m "insight-rebuild(module-18): record loop engineering evidence commit sha" >> "$LOG_FILE" 2>&1
...
if [ "$TEST_RC" -eq 0 ]; then
  summary "DONE: Insight Rebuild Module 18 tests passed. Evidence pushed."
  summary "Commit: $FINAL_SHA"
  summary "Evidence: reports/testing/latest_test_result.json; reports/execution/insight_rebuild_module_18_execution_report.md"
  exit 0
fi
summary "FAILED: Insight Rebuild Module 18 tests failed. Evidence pushed."
summary "Commit: $FINAL_SHA"
summary "Evidence: reports/testing/latest_test_result.json; reports/execution/insight_rebuild_module_18_execution_report.md"
exit "$TEST_RC"
