...
# Insight Rebuild Module 7 official bootstrap script.
# Policy: do not use set -e; terminal output stays short; detailed logs are written as GitHub evidence.

VERSION="2026-06-23-insight-rebuild-module-7-official"
...
summary "Insight Rebuild Module 7 official bootstrap $VERSION starting..."
...
git fetch origin >/tmp/insight_module_7_git.log 2>&1
git checkout "$BRANCH" >>/tmp/insight_module_7_git.log 2>&1
git pull --ff-only origin "$BRANCH" >>/tmp/insight_module_7_git.log 2>&1
rc=$?
...
LOG_FILE="$PRODUCT_DIR/reports/execution/insight_rebuild_module_7_bootstrap_latest.log"
{
  echo "# Insight Rebuild Module 7 bootstrap log"
  ...
  echo ""
  cat /tmp/insight_module_7_git.log 2>/dev/null || true
} > "$LOG_FILE" 2>&1
rm -f /tmp/insight_module_7_git.log

DUPLICATE_BOOTSTRAPS="$(find scripts -maxdepth 1 -type f -name 'bootstrap_insight_rebuild_module_7*.sh' ! -name 'bootstrap_insight_rebuild_module_7.sh' -print 2>> "$LOG_FILE")"
if [ -n "$DUPLICATE_BOOTSTRAPS" ]; then
  echo "duplicate_insight_rebuild_module_7_bootstraps=$DUPLICATE_BOOTSTRAPS" >> "$LOG_FILE"
  fail_short "duplicate Insight Rebuild Module 7 bootstrap entrypoints found; keep one official entry only"
fi

summary "Running Insight Rebuild Module 7 acceptance tests quietly..."
...
result = {
    "module": "Insight Rebuild Module 7",
    ...
    "runtime_v2_orchestration_adapter": True,
    "source_connector_boundary": True,
    "config_separated_settings": True,
    "deterministic_local_source_connector": True,
    "external_provider_boundary": "configured_but_disabled",
    "evidence_persistence_boundary": True,
    "log_file": os.environ.get("LOG_FILE", ""),
    "checks": [
        "api_surface_compatibility",
        "config_source_model_runtime_separation",
        "deterministic_connector_lens_coverage",
        "runtime_adapter_accepted_run",
        "empty_evidence_repair_path"
    ]
}
Path("reports/testing/insight_rebuild_module_7_test_result.json").write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
Path("reports/testing/latest_test_result.json").write_text(json.dumps(result, indent=2, ensure_ascii=False), encoding="utf-8")
PY

cat > reports/execution/insight_rebuild_module_7_execution_report.md <<EOF
...
Module 8 should harden durable evidence persistence, deployment smoke testing, or authenticated provider integration behind the Module 7 boundaries.
EOF

ensure_git_identity
git add reports/testing/insight_rebuild_module_7_test_result.json reports/testing/latest_test_result.json reports/execution/insight_rebuild_module_7_execution_report.md reports/execution/insight_rebuild_module_7_bootstrap_latest.log >> "$LOG_FILE" 2>&1
git commit -m "insight-rebuild(module-7): add acceptance evidence reports" >> "$LOG_FILE" 2>&1
rc=$?
if [ "$rc" -ne 0 ]; then fail_short "git commit failed for Module 7 evidence"; fi
EVIDENCE_SHA="$(git rev-parse HEAD 2>> "$LOG_FILE")"
"$PYTHON_BIN" - <<PY >> "$LOG_FILE" 2>&1
from pathlib import Path
p = Path("reports/execution/insight_rebuild_module_7_execution_report.md")
text = p.read_text(encoding="utf-8").replace("Pending until evidence commit completes.", "$EVIDENCE_SHA")
p.write_text(text, encoding="utf-8")
PY
git add reports/execution/insight_rebuild_module_7_execution_report.md reports/execution/insight_rebuild_module_7_bootstrap_latest.log >> "$LOG_FILE" 2>&1
git commit -m "insight-rebuild(module-7): record evidence commit sha" >> "$LOG_FILE" 2>&1
...
if [ "$TEST_RC" -eq 0 ]; then
  summary "DONE: Insight Rebuild Module 7 tests passed. Evidence pushed."
  summary "Commit: $FINAL_SHA"
  summary "Evidence: reports/testing/latest_test_result.json; reports/execution/insight_rebuild_module_7_execution_report.md"
  exit 0
fi
summary "FAILED: Insight Rebuild Module 7 tests failed. Evidence pushed."
summary "Commit: $FINAL_SHA"
summary "Evidence: reports/testing/latest_test_result.json; reports/execution/insight_rebuild_module_7_execution_report.md"
exit "$TEST_RC"
